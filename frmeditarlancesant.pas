...
uses
  Classes, SysUtils, FileUtil, DBDateTimePicker, DividerBevel,
  rxlookup, Forms, Controls,
  Graphics, Dialogs, ExtCtrls, StdCtrls, DBCtrls, Buttons, DBGrids, ComCtrls,
  frmzedicionbase, ZDataset, zcontroladoredicion, zdatasetgroup, SQLQueryGroup,
  DB, datGeneral, funciones, dateutils, LSExpression, LSConsts, LCLType, Math;
...
type
  ...
  TfmEditarLances = class(TZEdicionBase)
    ...
    zqPrincipalprofundidad_inicio_calado: TLongintField;
...
begin
  ...
  begin
    FLSExpression.Calculate('0' + Sender.AsString);
    if FLSExpression.Error = CLSExpressionNoError then
    begin
      zqPrincipalcaptura_babor.Value :=
        round(FLSExpression.Result / dmGeneral.zqMareaActivafactor_conversion.Value * 100) / 100;
    end;
  end;
  FLSExpression.Free;
end;
...
begin
  ...
  begin
    ...
    begin
      //Valido el rango
      factor := dmGeneral.zqMareaActivafactor_conversion.AsInteger;
      if (FLSExpression.Result < 0) or (FLSExpression.Result > factor) then
      begin
