...
uses
  ...
  {$IFDEF UNIX}
    Unix,
  {$ENDIF}
  Classes, SysUtils, FileUtil, LR_Class, LR_DBSet, lr_e_pdf, RxIniPropStorage,
  Forms, Controls, Graphics, Dialogs, ExtCtrls, Buttons, ActnList, ComCtrls,
  StdCtrls, EditBtn, IniPropStorage, Grids, frmbase, ZDataset, windirs, DB,
  datGeneral, comobj, variants, frmrptdatospuente, LR_Pars, LR_E_CSV, LR_E_TXT,
  LR_e_htmldiv, funciones, LSConfig;
...
type
  ...
  private
    ...
    procedure GenerarDatosPuente;
    procedure GenerarRindes;
    procedure GenerarCoccion;
    procedure GenerarTallas;
    procedure GenerarDanio;
    procedure GenerarByCatch;
    procedure GenerarByCatch2;
  public
    { public declarations }
  end;
...
begin
  ...
  begin
    ...
    if cbDanio.Checked then
      GenerarDanio;           GetUserDir;
    if cbByCatch.Checked then
    begin
      GenerarByCatch;
      GenerarByCatch2;
    end;
    if MessageDlg('Las planillas han sido guaradas en la carpeta indicada. ¿Desea abrir la carpeta para ver los archivos?', mtConfirmation, [mbYes, mbNo],0) = mrYes then
    begin
      {$IFDEF MSWINDOWS}
...
begin
  //FmRptDatosPuente.zqDatosPuente.Close;
  //FmRptDatosPuente.zqDatosPuente.Open;
  //FmRptDatosPuente.RLPDFFilter1.FileName:=dedCarpetaPlanillas.Directory+DirectorySeparator+'Datos de puente.pdf';
  //FmRptDatosPuente.rlrDatosPuente.Preview;
...
procedure TfmInformes.GenerarDatosPuente;
var
  xls: olevariant;
      ...
      begin
        ...
        begin
          ...
          if not FieldByName('captura_babor').IsNull then
             xls.Cells[fila, 13] := FieldByName('captura_babor').AsFloat;
          if not FieldByName('captura_estribor').IsNull then
             xls.Cells[fila, 14] := FieldByName('captura_estribor').AsFloat;
...
procedure TfmInformes.GenerarRindes;
var
  xls: olevariant;
  archivo_origen, archivo_destino, tmp: WideString;
  fila, columna: integer;
...
procedure TfmInformes.GenerarCoccion;
var
  xls: olevariant;
  archivo_origen, archivo_destino, tmp: WideString;
  fila, columna: integer;
...
procedure TfmInformes.GenerarTallas;
var
  xls: olevariant;
  archivo_origen, archivo_destino, tmp: WideString;
...
procedure TfmInformes.GenerarDanio;
var
  xls, xlp: olevariant;
...
procedure TfmInformes.GenerarByCatch;
var
  xls, xlp: olevariant;
  archivo_origen, archivo_destino, plantilla, tmp, rango: WideString;
  fila, columna: integer;
...
procedure TfmInformes.GenerarByCatch2;
var
  xls, xlp: olevariant;
  archivo_origen, archivo_destino, plantilla, tmp, rango: WideString;
